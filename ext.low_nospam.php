	...
	function sessions_start(&$session)
	{
		...
		if ($this->settings['zero_tolerance'] == 'y' && (REQ == 'ACTION' || REQ == 'PAGE') && $_POST)
		{
			...
			foreach ($_POST AS $key => $value)
			{
				...
				// Fill author spot
				if (isset($post['comment_author']) && ! $post['comment_author'] && in_array($key, array('name', 'username', 'screen_name', 'author')))
				{
				...
				// Fill email
				if (isset($post['comment_author_email']) && ! $post['comment_author_email'] && in_array($key, array('mail', 'email', 'emailaddress', 'e-mail', 'from', 'to')))
				{
				...
				// Fill url
				if (isset($post['comment_author_url']) && ! $post['comment_author_url'] && in_array($key, array('url', 'site', 'website')))
				{
